...
class ToolMonitor:
    ...
    def report_approval_required(
        ...
        """向客户端推送审批请求"""
        self._emit("approval_required", message or f"需要审批: {approval_type}", {
            "approval_id": approval_id,
            "approval_type": approval_type,
            **payload,
        })

    def report_approval_resolved(self, approval_id: str, action: str):
        """通知客户端审批已处理"""
        self._emit("approval_resolved", f"审批已处理: {action}", {
            "approval_id": approval_id,
            "action": action,
        })
