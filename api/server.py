...
from agent.main_agent import run_main_agent
...
# 配置 CORS
app.add_middleware(
    CORSMiddleware,#请求进入前先处理，响应返回前再处理
    allow_origins=['*'
    ],#允许需要的前端访问，否则浏览器会拦截请求
...
async def run_task(request: TaskRequest):
    ...
    asyncio.create_task(run_main_agent(request.query, thread_id))

    # 3. [立即响应]
    return {"status": "started", "thread_id": thread_id}


@app.post("/api/upload")#上传多个文件,File(...)必填参数，
                        # 从form-data种获取thread_id，请求头中Content-Type:multipart/form-data用于文件上传
async def upload_files(files: List[UploadFile] = File(...), thread_id: str = Form(...)):
    ...
    for file in files:
        ...
        with file_path.open("wb") as buffer:#wb就是二进制写入writer byte
            shutil.copyfileobj(file.file, buffer)#流式复制
...
async def download_file(path: str):
    ...
    try:
        ...
        # 必须确保请求的文件在 output 目录下，防止被下载系统文件
    ...
    # 3. dastapi自动[响应] 返回文件流 (浏览器自动触发下载)
...
async def list_files(path: str):
    ...
    try:
        # 5. [遍历] 递归查找所有文件
        for file_path in abs_path.rglob("*"):#rglob("*")递归查找全部
            if file_path.is_file():#递归遍历所有文件，
                # 计算相对路径，生成下载 URL
                stat = file_path.stat()
                files.append({
                    "name": file_path.name,
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "ping":
                ...
                action = msg.get("action", "reject")
                result = {
                    "action": action,
                    "reason": msg.get("reason", ""),
                    "modifications": msg.get("modifications", {})
                }
                success = approval_registry.resolve(approval_id, result)
                if success:
                    print(f"[HITL] 审批 {approval_id[:8]} 已处理: {action}")
                    # 直接通过当前 WebSocket 连接发回审批结果（不能用 monitor，
                    # 因为 monitor._emit 依赖 ContextVar 路由，此处 ContextVar 为空）
                    await websocket.send_json({
                        "type": "monitor_event",
                        "event": "approval_resolved",
                        "message": f"审批已处理: {action}",
                        "data": {
                    ...
                    await websocket.send_json({
                        "type": "error",
                        "message": f"审批 {approval_id} 不存在或已过期"
                    })
        ...
        # HITL: 断连时取消该会话所有待审批项
        cancelled = approval_registry.cancel_all_for_thread(thread_id)
        if cancelled > 0:
            print(f"[HITL] 会话 {thread_id} 断连，已取消 {cancelled} 个待审批项")
        manager.disconnect(websocket, thread_id)
        print(f"[WebSocket] 客户端已断开: {thread_id}")

    except Exception as e:
        # HITL: 异常时也取消该会话所有待审批项
        cancelled = approval_registry.cancel_all_for_thread(thread_id)
        if cancelled > 0:
            print(f"[HITL] 会话 {thread_id} 异常，已取消 {cancelled} 个待审批项")
        print(f"[WebSocket] 连接异常: {e}")
        manager.disconnect(websocket, thread_id)


@app.post("/api/approval/respond")
async def respond_approval(request: dict):
    """
    审批响应 HTTP 降级接口（WebSocket 不稳定时的备用方案）。
    """
    ...
    success = approval_registry.resolve(approval_id, result)
    if success:
        monitor.report_approval_resolved(approval_id, action)
