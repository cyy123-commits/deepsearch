...
const handleSocketMessage = (data: any) => {
  ...
  if (event === 'session_created') {
    ...
    pendingApproval.value = {
      ...
      approval_type: eventData.approval_type,
      payload: eventData,
    ...
    if (lastAiMsg) {
      ...
      lastAiMsg.logs.push({
        ...
        title: `等待审批: ${eventData.approval_type}...`,
        details: eventData,
        timestamp: new Date().toLocaleTimeString()
      })
    }
  } else if (event === 'approval_resolved') {
    // HITL: dismiss approval card
...
// HITL: send approval response back to server
const sendApprovalResponse = (action: 'approve' | 'reject') => {
  if (!pendingApproval.value || !socket.value) return
          ...
          <div class="approval-card">
            ...
            <div class="approval-body">
              <div class="warning-banner">
                检测到数据库写入操作 ({{ pendingApproval.payload.operation }})
              </div>
              <pre class="sql-preview"><code>{{ pendingApproval.payload.query }}</code></pre>
            </div>
            <div class="approval-actions">
              <input v-model="approvalReason" placeholder="审批意见（可选）" class="approval-reason" />
              <div class="approval-buttons">
                <button class="btn-approve" @click="sendApprovalResponse('approve')">批准</button>
                <button class="btn-reject" @click="sendApprovalResponse('reject')">拒绝</button>
              </div>
            </div>
          </div>
        </div>
